...
- **Agent Builder canonical design (resolved, 2026-06-25):** the real Simple-Agent builder is **two-pane** (config left + live chat preview right); the header has a **center segmented control** (AI Assisted | Manual configuration) + Run **toggle** + Save + `⋮`; page tabs are **Configuration | Deployments | Observability** (pill); the config pane is a **flat stack of titled sections with dashed add-zones — not cards**; Instructions is a compact input w/ expand icon, not a textarea; the preview pane has the agent identity empty state + a chat composer. See `production-layout-spec.md` Reference 3. This **scopes design-rules §4** (cards → data-entry forms only; builder panes are flat sections) and **§9** (dashed add-zones are functional inputs, allowed). The trial frame (single column of cards) is superseded as the reference.
